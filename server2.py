...
class SubtitleRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/get_subtitle"):
            ...
            # 쿼리 파라미터에서 언어 코드를 가져오기
            parsed_path = urllib.parse.urlparse(self.path)
            query_params = urllib.parse.parse_qs(parsed_path.query)
            target_language = query_params.get('lang', ['ko'])[0]  # 기본 언어를 한국어로 설정

            # 자막 텍스트를 선택된 언어로 번역
            translated_text = translate_text(transcript_text, target_language=target_language)
